...
```bash
docker exec -it lab-zero-trust-glpi /bin/sh
```
...
### Teste a conexão usando SSH nos diferentes containers:

```bash
ssh -i /home/minicurso/.ssh/id_rsa minicurso@lab-zero-trust-mariadb nmap
ssh -i /home/minicurso/.ssh/id_rsa minicurso@lab-zero-trust-backup nmap
```
